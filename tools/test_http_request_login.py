...
class TestHttpRequest_Login(unittest.TestCase):
    ...
    #     # print(eval(item["header"]))
        res = HttpRequest.http_request(item['url'],item['data'],eval(item["header"]),eval(item["params"]),item['http_method'],getattr(GetData,"Cookie"))
        if res.cookies:
...
if __name__ == '__main__':
    '''从这里开始测试'''
    TestHttpRequest_Login().test_api()
